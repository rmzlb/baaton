...
/// Each event is a JSON string with { type, ... } structure.
...
pub async fn event_stream(
    ...
    let rx = tx.subscribe();
    let user_org_id = auth.org_id.unwrap_or_default();

    let stream = BroadcastStream::new(rx)
        .filter_map(move |msg| {
            match msg {
                Ok(evt) if evt.org_id == user_org_id => {
                    Some(Ok(Event::default().data(evt.payload)))
                }
                _ => None,
            }
        });
...
/// Helper to broadcast an event (fire-and-forget, ignores errors).
