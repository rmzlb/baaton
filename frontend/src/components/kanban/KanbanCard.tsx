...
const CORE_STATUS_LABELS: Record<string, string> = {
  ...
  not_ok: 'Not OK', in_review: 'In Review', done: 'Done', cancelled: 'Canceled',
};
  ...
  const SelectCheckbox = onSelect ? (
    <span
      ...
      className={cn(
        ...
        selected ? 'bg-accent border-accent text-black opacity-100' : 'border-foreground/10 bg-card opacity-0 group-hover/card:opacity-100',
      )}
    >
