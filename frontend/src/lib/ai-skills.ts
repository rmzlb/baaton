...
export const ALL_SKILL_DECLARATIONS = [
      {
        ...
        parameters: {
          ...
          properties: {
            query: {
              type: 'STRING',
              description: 'Free-text search string matched against issue title and description. Leave empty to list all issues with only filter-based criteria. Example: "auth token refresh" or "onboarding bug".',
            },
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to search within. Omit to search across all projects the user has access to.',
            },
            status: {
              type: 'STRING',
              description: 'Filter to issues in a specific workflow status. Valid values: backlog, todo, in_progress, in_review, done, cancelled. Omit to include all statuses.',
            },
            priority: {
              type: 'STRING',
              description: 'Filter to issues of a specific priority level. Valid values: urgent, high, medium, low. Omit to include all priorities.',
            },
            category: {
              type: 'STRING',
              description: 'Filter to issues tagged with a specific technical domain. Valid values: FRONT, BACK, API, DB, INFRA, UX, DEVOPS. Omit to include all categories.',
            },
            limit: {
              type: 'NUMBER',
              description: 'Maximum number of results to return. Defaults to 20. Increase up to 100 for broader scans; decrease to 5 for quick lookups.',
            },
          },
          required: [],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to create the issue in. Required — never omit.',
            },
            title: {
              type: 'STRING',
              description: 'Short, action-oriented issue title. Must be plain text with no brackets, prefixes, or type tags. Examples: "Fix auth token refresh on expiry", "Add CSV export to billing page", "Migrate users table to new schema".',
            },
            description: {
              type: 'STRING',
              description: 'Detailed issue description in Markdown format. Include: reproduction steps for bugs, acceptance criteria for features, technical context, and any relevant links. Leave empty only if no additional context exists.',
            },
            type: {
              type: 'STRING',
              description: 'Issue classification. Valid values: bug (something broken), feature (new capability), improvement (enhancement to existing behavior), question (clarification needed). Infer from context when not explicitly stated.',
            },
            priority: {
              type: 'STRING',
              description: 'Urgency level. Valid values: urgent (blocking production or users), high (important, next sprint), medium (normal priority), low (nice-to-have or deferred). Infer from language cues like "critical", "blocking", "when possible".',
            },
            status: {
              type: 'STRING',
              description: 'Initial workflow status. Valid values: backlog, todo, in_progress, in_review, done, cancelled. Default to backlog unless the user explicitly says the work has started (→ in_progress) or is already queued (→ todo).',
            },
            tags: {
              type: 'ARRAY',
              items: { type: 'STRING' },
              description: 'Free-form labels to aid filtering and grouping. Examples: ["auth", "mobile", "regression"]. Use lowercase, hyphenated strings.',
            },
            category: {
              type: 'ARRAY',
              items: { type: 'STRING' },
              description: 'Technical domain(s) the issue belongs to. Valid values: FRONT (React/UI), BACK (server logic), API (REST/GraphQL endpoints), DB (database/migrations), INFRA (Docker/CI/CD), UX (design/accessibility), DEVOPS (deployment/monitoring). Auto-detect from description keywords; multiple values allowed.',
            },
          },
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            issue_id: {
              type: 'STRING',
              description: 'Internal UUID of the issue to update (not the display_id like HLM-42). Resolve via search_issues first if you only have the display_id.',
            },
            title: {
              type: 'STRING',
              description: 'New plain-text title, following the same no-brackets rule as create_issue. Omit if not changing.',
            },
            description: {
              type: 'STRING',
              description: 'New description in Markdown format. Omit if not changing. This replaces the full existing description.',
            },
            status: {
              type: 'STRING',
              description: 'New workflow status. Valid values: backlog, todo, in_progress, in_review, done, cancelled.',
            },
            priority: {
              type: 'STRING',
              description: 'New priority level. Valid values: urgent, high, medium, low.',
            },
            type: {
              type: 'STRING',
              description: 'New issue classification. Valid values: bug, feature, improvement, question.',
            },
            tags: {
              type: 'ARRAY',
              items: { type: 'STRING' },
              description: 'New complete set of tags. This REPLACES existing tags, not appends. Pass the full desired tag list.',
            },
            category: {
              type: 'ARRAY',
              items: { type: 'STRING' },
              description: 'New complete set of technical domains. Valid values: FRONT, BACK, API, DB, INFRA, UX, DEVOPS. Replaces existing categories.',
            },
          },
          required: ['issue_id'],
        },
            ...
            updates: {
              ...
              items: {
                ...
                properties: {
                  ...
                  status: {
                    type: 'STRING',
                    description: 'New workflow status: backlog, todo, in_progress, in_review, done, cancelled.',
                  },
                  priority: {
                    type: 'STRING',
                  ...
                  category: {
                    type: 'ARRAY',
                    items: { type: 'STRING' },
                    description: 'New technical domains: FRONT, BACK, API, DB, INFRA, UX, DEVOPS (replaces existing).',
                  },
                },
                required: ['issue_id'],
              },
              description: 'Array of per-issue update objects. Each entry must include issue_id; all other fields are optional and only specified ones will be changed.',
            },
          },
          required: ['updates'],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            issue_id: {
              type: 'STRING',
              description: 'Internal UUID of the issue to comment on. Resolve via search_issues if you only have the display_id.',
            },
            content: {
              type: 'STRING',
              description: 'Comment body in Markdown format. Supports headers, code blocks, bullet lists, and bold/italic. Example: "Investigated the 500 error — root cause is a missing null check in the session middleware."',
            },
            author_name: {
              type: 'STRING',
              description: 'Display name of the comment author shown in the UI. Defaults to the authenticated user if omitted. Use when posting on behalf of someone else.',
            },
          },
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            brief: {
              type: 'STRING',
              description: 'Short description of the feature or problem to document. Can be a sentence to a paragraph. Example: "Add a CSV export button on the billing page so admins can download invoice history."',
            },
            project_id: {
              type: 'STRING',
              description: 'UUID of the project this PRD belongs to. Used to pull existing issue context (tech stack, naming conventions) that will make the PRD more specific. Omit for generic PRDs.',
            },
          },
          required: ['brief'],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to analyze. Omit to analyze all active projects together and surface cross-project velocity patterns.',
            },
          },
          required: [],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to fetch metrics for. Omit to aggregate metrics across all projects the user has access to.',
            },
          },
          required: [],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to recap. Omit to generate a cross-project recap covering all active projects.',
            },
            days: {
              type: 'NUMBER',
              description: 'Number of calendar days to look back. Defaults to 7 (one week). Use 1 for a daily standup, 14 for a bi-weekly review, or 30 for a monthly summary.',
            },
          },
          required: [],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to analyze. Omit to generate cross-project priority recommendations.',
            },
          },
          required: [],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to plan milestones for. Required.',
            },
            target_date: {
              type: 'STRING',
              description: 'Hard deadline in ISO 8601 format (YYYY-MM-DD). When provided, the milestone plan is compressed or reordered to ensure all milestones land before this date. Example: "2026-04-30".',
            },
            team_size: {
              type: 'NUMBER',
              description: 'Number of developers actively working on this project. Defaults to 1. Increasing this value shortens estimated milestone durations proportionally. Example: 3 for a three-person team.',
            },
          },
          required: ['project_id'],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to create milestones in.',
            },
            milestones: {
              type: 'ARRAY',
              items: {
                ...
                properties: {
                  name: {
                    type: 'STRING',
                    description: 'Milestone name shown in the UI. Should be short and meaningful. Example: "Alpha Release", "Auth Hardening", "Tech Debt Q1".',
                  },
                  description: {
                    type: 'STRING',
                    description: 'Optional markdown description of what this milestone represents and its goal.',
                  },
                  target_date: {
                    type: 'STRING',
                    description: 'Target completion date in YYYY-MM-DD format. Example: "2026-04-15".',
                  },
                  order: {
                    type: 'NUMBER',
                    description: 'Display order (1-based integer). Milestones are shown in ascending order in the UI.',
                  },
                  issue_ids: {
                    type: 'ARRAY',
                    items: { type: 'STRING' },
                    description: 'Array of internal issue UUIDs to assign to this milestone. Use UUIDs from plan_milestones output or search_issues results.',
                  },
                },
                required: ['name', 'issue_ids'],
              },
              description: 'Ordered list of milestones to create. Typically sourced directly from the plan_milestones output after user confirmation.',
            },
          },
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project whose timeline to adjust.',
            },
            constraint: {
              type: 'STRING',
              description: 'Natural-language description of the new constraint or deadline change. Examples: "finish everything by 2026-03-15", "move milestone Alpha two weeks earlier", "we now have only 2 devs instead of 3", "drop the Tech Debt milestone from scope".',
            },
          },
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            issue_id: {
              type: 'STRING',
              description: 'Internal UUID of the issue to triage. Resolve via search_issues if you only have the display_id (e.g. HLM-42).',
            },
          },
          required: ['issue_id'],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            action: {
              type: 'STRING',
              description: 'Operation to perform. Valid values: list (show all initiatives), create (new initiative), update (change fields), add_project (link a project), remove_project (unlink a project).',
            },
            initiative_id: {
              type: 'STRING',
              description: 'UUID of the initiative to modify. Required for update, add_project, and remove_project actions. Omit for list and create.',
            },
            name: {
              type: 'STRING',
              description: 'Initiative name. Required for create; optional for update. Example: "Q2 Product Launch".',
            },
            description: {
              type: 'STRING',
              description: 'Markdown description of the initiative\'s goal and scope. Used in create and update actions.',
            },
            status: {
              type: 'STRING',
              description: 'Lifecycle status for update action. Valid values: active (in progress), completed (finished), archived (no longer relevant).',
            },
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to link or unlink. Required for add_project and remove_project actions.',
            },
          },
          required: ['action'],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            action: {
              type: 'STRING',
              description: 'Operation to perform. Valid values: list (show all automations for the project), create (new rule), toggle (enable/disable an existing rule), delete (remove a rule permanently).',
            },
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to manage automations for. Required for all actions.',
            },
            automation_id: {
              type: 'STRING',
              description: 'UUID of the automation rule to toggle or delete. Required for toggle and delete actions.',
            },
            name: {
              type: 'STRING',
              description: 'Human-readable name for the automation. Used in create. Example: "Auto-escalate stale urgents".',
            },
            trigger_type: {
              type: 'STRING',
              description: 'Event that activates this automation. Valid values: status_changed, priority_changed, assignee_changed, label_added, due_date_passed. Required for create.',
            },
            trigger_config: {
              type: 'STRING',
              description: 'JSON string with trigger-specific parameters. For status_changed: {"from_status":"todo","to_status":"in_progress"}. For priority_changed: {"to_priority":"urgent"}. For label_added: {"label":"regression"}.',
            },
            action_type: {
              type: 'STRING',
              description: 'What happens when the trigger fires. Valid values: set_status, set_priority, add_label, assign_user, send_webhook, add_comment. Required for create.',
            },
            action_config: {
              type: 'STRING',
              description: 'JSON string with action-specific parameters. For set_status: {"status":"in_review"}. For set_priority: {"priority":"high"}. For add_comment: {"content":"Auto-assigned to review queue"}. For send_webhook: {"url":"https://hooks.example.com/notify"}.',
            },
          },
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            action: {
              type: 'STRING',
              description: 'Operation to perform. Valid values: list_rules (show all SLA rules), stats (SLA compliance metrics), create_rule (add a new priority→deadline rule), delete_rule (remove a rule).',
            },
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to manage SLA rules for. Required for all actions.',
            },
            rule_id: {
              type: 'STRING',
              description: 'UUID of the SLA rule to delete. Required for delete_rule action only.',
            },
            priority: {
              type: 'STRING',
              description: 'Priority level this rule applies to. Valid values: urgent, high, medium, low. Required for create_rule.',
            },
            deadline_hours: {
              type: 'NUMBER',
              description: 'Maximum allowed resolution time in hours for the specified priority. Required for create_rule. Examples: 4 (urgent), 24 (high), 72 (medium), 168 (low/one week).',
            },
          },
          required: ['action', 'project_id'],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            action: {
              type: 'STRING',
              description: 'Operation to perform. Valid values: list (show all templates for the project), create (add a new template), delete (remove a template permanently).',
            },
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to manage templates for. Required for all actions.',
            },
            template_id: {
              type: 'STRING',
              description: 'UUID of the template to delete. Required for delete action only.',
            },
            name: {
              type: 'STRING',
              description: 'Display name for the template shown in the issue creation UI. Required for create. Example: "Bug Report", "Feature Request", "Security Incident".',
            },
            description: {
              type: 'STRING',
              description: 'Pre-filled Markdown body that issues created from this template will have. Should include structured sections like "## Steps to Reproduce", "## Expected Behavior", "## Acceptance Criteria". Required for create.',
            },
            default_priority: {
              type: 'STRING',
              description: 'Default priority pre-selected when using this template. Valid values: urgent, high, medium, low. Used in create.',
            },
            default_type: {
              type: 'STRING',
              description: 'Default issue type pre-selected when using this template. Valid values: bug, feature, improvement, question. Used in create.',
            },
          },
          required: ['action', 'project_id'],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            action: {
              type: 'STRING',
              description: 'Operation to perform. Valid values: list (show all recurring configs), create (define a new one), toggle (pause/resume), trigger (run immediately), delete (remove permanently).',
            },
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to manage recurring issue configs for. Required for all actions.',
            },
            recurring_id: {
              type: 'STRING',
              description: 'UUID of the recurring config to toggle, trigger, or delete. Required for toggle, trigger, and delete actions.',
            },
            title: {
              type: 'STRING',
              description: 'Title template for the auto-created issues. Required for create. Example: "Weekly Security Review", "Daily Dependency Scan".',
            },
            description: {
              type: 'STRING',
              description: 'Markdown description body for the auto-created issues. Required for create. Should include checklist or instructions for the recurring task.',
            },
            priority: {
              type: 'STRING',
              description: 'Priority assigned to each auto-created issue. Valid values: urgent, high, medium, low. Required for create.',
            },
            issue_type: {
              type: 'STRING',
              description: 'Type assigned to each auto-created issue. Valid values: bug, feature, improvement, question. Required for create.',
            },
            cron_expression: {
              type: 'STRING',
              description: 'Standard 5-field cron expression controlling the schedule. Required for create. Examples: "0 9 * * 1" (every Monday 9am), "0 0 1 * *" (first of every month midnight), "0 8 * * 1-5" (weekdays 8am). Uses UTC timezone.',
            },
          },
          required: ['action', 'project_id'],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            project_id: {
              type: 'STRING',
              description: 'UUID of the project to export. All issues in this project will be included in the export payload.',
            },
          },
          required: ['project_id'],
        },
